...
export function SupportDialog() {
    ...
    const [formData, setFormData] = useState({
        ...
        contact: '',
        message: '',
        roomNumber: '', // Added for Lab Add Request
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (selectedCategory.title !== 'Lab Add Request' && (formData.message.length < 3 || formData.message.length > 1500)) {
            toast.error("Invalid Message", {
                description: "Message must be between 3 and 1500 characters."
            });
            setIsSubmitting(false);
            return;
        }

        // Lab Add Request Validation
        if (selectedCategory.title === 'Lab Add Request') {
            if (formData.roomNumber.length > 3) {
                toast.error("Invalid Room Number", {
                    description: "Room number cannot exceed 3 digits."
                });
                setIsSubmitting(false);
                return;
            }
            ...
            if (roomNumbers.includes(formData.roomNumber)) {
                toast.error("Room Already Exists", {
                    description: `Room ${formData.roomNumber} is already available in the list.`
                });
                setIsSubmitting(false);
                return;
            }
        ...
        try {
            ...
            const payload = {
                ...
                message: selectedCategory.title === 'Lab Add Request' ? '' : formData.message, // Empty message for Lab Request
                roomNumber: selectedCategory.title === 'Lab Add Request' ? formData.roomNumber : '', // Only for Lab Request
    ...
    const options = [
        ...
        },
        {
            ...
            description: 'Help me improve ShareMe',
            color: 'text-amber-500 bg-amber-500/10 hover:bg-amber-500/20'
        },
        {
            icon: Monitor,
            title: 'Lab Add Request',
            description: 'Want to add your lab room?',
    ...
    return (
        <Dialog open={open} onOpenChange={(val) => {
            ...
            <DialogContent className="w-[95vw] max-w-md border-0 bg-background/95 backdrop-blur-xl max-h-[85vh] overflow-y-auto p-4 sm:p-6 rounded-2xl">
                    ...
                    {view === 'form' && selectedCategory && (
                        ...
                        >
                            ...
                            <div className="space-y-3">
                                ...
                                {/* Conditional Field: Room Number for Lab Request vs Message for others */}
